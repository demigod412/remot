...
@section('page-title', 'Submit Proof')

@section('content')
<div style="display:flex; align-items:center; gap:6px; font-size:12px; color:var(--fg-3); margin-bottom:20px;">
    <a href="{{ route('user.submissions.index') }}" style="color:var(--fg-3); text-decoration:none;"
...
<div style="display:flex; flex-direction:column; gap:14px;">
    ...
    <div class="card" style="padding:18px; display:flex; gap:14px; align-items:flex-start;">
        @if($submission->work->cover_image)
        <img src="{{ fileUrl(config('jobstation.upload_paths.work_cover'), $submission->work->cover_image) }}"
             style="width:60px; height:60px; border-radius:10px; object-fit:cover; flex-shrink:0;">
        @endif
        <div style="flex:1; min-width:0;">
            <div style="font-size:15px; font-weight:600; color:var(--fg);">{{ $submission->work->title }}</div>
            ...
            <div style="font-size:11px; margin-top:4px; display:flex; align-items:center; gap:4px;
                color:{{ $submission->deadline_at->isPast() ? 'var(--danger)' : ($submission->deadline_at->diffInHours(now()) <= 6 ? 'var(--warn)' : 'var(--fg-3)') }};">
                <i data-lucide="clock" style="width:11px; height:11px;"></i>
                @if($submission->deadline_at->isPast())
                    Auto-approved (deadline passed)
                @else
                    Auto-approves in
                    <span id="proof-deadline"
                          data-expires="{{ $submission->deadline_at->timestamp }}"
                          style="font-weight:600;font-family:var(--font-mono);">{{ $submission->deadline_at->diffForHumans() }}</span>
                @endif
            </div>
            @endif
        </div>
    ...
    <div class="card" style="padding:18px;">
        ...
        <div style="font-size:13px; color:var(--fg-2); line-height:1.65;">
            {!! richBody($submission->work->description) !!}
        </div>
    </div>
    ...
    <div class="card" style="padding:18px;"
        ...
        <form method="POST" action="{{ route('user.submissions.proof.submit', $submission->id) }}"
            ...
            <div>
                ...
                <textarea name="proof_note" rows="5"
                          style="{{ $errors->has('proof_note') ? 'border-color:var(--danger);' : '' }} resize:vertical;"
                          placeholder="Describe what you did and how you completed the task. Include any relevant links, usernames, etc."
                          required>{{ old('proof_note') }}</textarea>
                <div style="font-size:11px; color:var(--fg-4); margin-top:4px;">Minimum 20 characters</div>
                @error('proof_note') <div style="font-size:12px; color:var(--danger); margin-top:4px;">{{ $message }}</div> @enderror
            </div>
            ...
            <div>
                ...
                @error('proof_files') <div style="font-size:12px; color:var(--danger); margin-top:6px;">{{ $message }}</div> @enderror
                @error('proof_files.*') <div style="font-size:12px; color:var(--danger); margin-top:6px;">{{ $message }}</div> @enderror
            </div>
            ...
            <div style="display:flex; gap:10px; padding-top:6px;">
                ...
                <a href="{{ route('user.submissions.index') }}" class="btn" style="padding:9px 20px; text-decoration:none;">Cancel</a>
            </div>
        </form>
    </div>
