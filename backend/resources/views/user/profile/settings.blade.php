...
@section('page-title', 'Profile & KYC')
    ...
    <div>
        ...
        <div class="card" style="padding:24px; margin-bottom:16px;">
            <div style="display:flex; align-items:center; gap:18px; margin-bottom:24px; flex-wrap:wrap;">
                <div style="width:72px; height:72px; border-radius:50%; background:linear-gradient(135deg,{{ $uColor }},{{ $uColor2 }}); display:flex; align-items:center; justify-content:center; color:white; font-weight:700; font-size:28px; flex-shrink:0;">
                    {{ $uInit }}
                </div>
                <div style="flex:1; min-width:0;">
                    <div style="display:flex; align-items:center; gap:10px; margin-bottom:4px; flex-wrap:wrap;">
                        <h2 style="font-size:22px; font-weight:600; margin:0; letter-spacing:-0.4px; color:var(--fg);">{{ $uName }}</h2>
                        @if($kyc == 1)
                    ...
                    <div style="font-size:13px; color:var(--fg-3);">{{ '@' . $user->username }}@if($user->country_code) · {{ $user->country_code }}@endif · Joined {{ $user->created_at->format('M Y') }}</div>
                </div>
            </div>
            ...
            <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px; padding:20px 0; border-top:1px solid var(--border); border-bottom:1px solid var(--border);" class="profile-stats-row">
                <div>
                    ...
                    <div class="mono" style="font-size:18px; font-weight:600;">{{ $approvalRate }}%</div>
                    <div style="font-size:10.5px; color:var(--fg-3);">{{ $approvedCount }} approved</div>
                </div>
                <div>
                    <div class="label" style="margin-bottom:4px;">Tasks done</div>
                    <div class="mono" style="font-size:18px; font-weight:600;">{{ $approvedCount }}</div>
                ...
                <div>
                    ...
                    <div class="mono" style="font-size:18px; font-weight:600;">{{ $progress }}%</div>
            ...
            <div style="margin-top:20px;">
                ...
                <div style="display:flex; gap:6px; flex-wrap:wrap;">
                    @forelse($skills->whereIn('id', $userSkillIds) as $skill)
        ...
        <div class="card" style="padding:24px; margin-bottom:16px;">
            ...
            <form method="POST" action="{{ route('user.profile.update') }}" enctype="multipart/form-data">
                ...
                @endif
                @if($errors->any())
                <div style="padding:10px 14px; background:rgba(239,68,68,0.1); border:1px solid rgba(239,68,68,0.25); border-radius:8px; font-size:13px; color:#FCA5A5; margin-bottom:16px;">{{ $errors->first() }}</div>
                @endif
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;" class="form-2col">
                    <div>
                        ...
                        <input type="text" name="country_code" value="{{ old('country_code', $user->country_code) }}" placeholder="e.g. TZ">
                    </div>
                    <div>
                        <label style="display:block; font-size:12px; color:var(--fg-2); margin-bottom:6px; font-weight:500;">Mobile</label>
                        <input type="text" name="mobile" value="{{ old('mobile', $user->mobile) }}" placeholder="+255...">
                    </div>
                </div>
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <div style="display:flex; flex-wrap:wrap; gap:8px;" id="skill-list">
                        ...
                        <label style="cursor:pointer; user-select:none;">
                            <input type="checkbox" name="skills[]" value="{{ $skill->id }}" {{ $checked ? 'checked' : '' }} style="display:none;" class="skill-cb">
                            <span class="chip skill-chip {{ $checked ? 'skill-active' : '' }}" style="{{ $checked ? 'background:var(--accent-soft);color:var(--accent);border-color:rgba(47,84,235,0.35);' : '' }}">{{ $skill->name }}</span>
                        </label>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="card" style="padding:24px;">
            ...
            <div style="display:flex; align-items:center; gap:14px; padding:14px 16px; background:rgba(239,68,68,0.06); border-radius:10px; border:1px solid rgba(239,68,68,0.2); margin-bottom:20px;">
                ...
                <a href="{{ route('user.profile.kyc') }}" class="btn btn-primary" style="font-size:12px;padding:7px 14px;flex-shrink:0;">Verify →</a>
            </div>
            @endif
            ...
            <div style="display:flex; flex-direction:column;">
                ...
                @endphp
                @foreach($kycRows as $i => $row)
                <div style="display:flex; align-items:center; gap:14px; padding:12px 0; border-bottom:{{ $i < 3 ? '1px solid var(--border)' : 'none' }};">
                    <span style="width:28px; height:28px; border-radius:50%; display:flex; align-items:center; justify-content:center; flex-shrink:0;
                                 background:{{ $row['done'] ? 'rgba(34,197,94,0.12)' : ($row['icon'] === 'clock' ? 'rgba(245,158,11,0.1)' : 'var(--surface-2)') }};
                                 color:{{ $row['done'] ? '#22C55E' : ($row['icon'] === 'clock' ? '#F59E0B' : 'var(--fg-4)') }};">
                        <i data-lucide="{{ $row['icon'] }}" style="width:13px; height:13px;"></i>
                    </span>
                    <div style="flex:1; min-width:0;">
                        <div style="font-size:13px; font-weight:500; color:var(--fg);">{{ $row['title'] }}</div>
                        <div style="font-size:11.5px; color:var(--fg-3);">{{ $row['detail'] }}</div>
                    </div>
    ...
    <div>
        <div class="card" style="padding:20px; margin-bottom:14px;">
            ...
            <div style="text-align:center; padding:8px 0 16px;">
                <div style="font-size:32px; margin-bottom:4px;">{{ $rank['icon'] }}</div>
                <div style="font-size:18px; font-weight:600; color:var(--fg);">{{ $rank['label'] }}</div>
                <div style="font-size:12px; color:var(--fg-3);">{{ $rank['score'] }} pts</div>
            </div>
            ...
            <div style="display:flex; justify-content:space-between; font-size:11px; color:var(--fg-3);">
                <span>{{ $rank['label'] }}</span><span>{{ $progress }}% to next</span>
            </div>
        </div>
        ...
        <div class="card" style="padding:20px; margin-bottom:14px;">
            ...
            <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:8px;">
                ...
                <div style="padding:10px; background:var(--surface-2); border-radius:8px; text-align:center;">
                    <i data-lucide="{{ $b[2] }}" style="width:18px; height:18px; color:{{ $b[1] }}; margin:0 auto 4px; display:block;"></i>
                    <div style="font-size:10.5px; color:var(--fg-2);">{{ $b[0] }}</div>
                </div>
                @empty
        ...
        <div class="card" style="padding:20px; margin-bottom:14px;">
            ...
            <a href="{{ route('user.profile.password') }}" class="btn" style="width:100%; justify-content:center; font-size:13px; display:flex; align-items:center; gap:6px;">
                <i data-lucide="lock" style="width:13px; height:13px;"></i> Change password
            </a>
        </div>

        @if(Route::has('user.referral.index'))
        <div class="card" style="padding:20px;">
            ...
            <a href="{{ route('user.referral.index') }}" class="btn" style="width:100%; justify-content:center; font-size:13px; display:flex; align-items:center; gap:6px;">
