...
@section('subheading', 'Sign in to pick up where you left off — running tasks, active applications, and your wallet.')

@section('top-right')
<div style="font-size:13px;color:var(--fg-3);">
    New to {{ gs()->site_name ?? 'Job Station' }}?
    <a href="{{ route('user.register') }}" style="color:#2f54eb;font-weight:500;text-decoration:none;">Create account →</a>
</div>
@endsection
...
<form method="POST" action="{{ route('user.login.submit') }}">
    ...
    <div class="auth-field" x-data="{ show: false }">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            ...
            <a href="{{ route('user.forgot-password') }}" style="font-size:12px;color:#2f54eb;text-decoration:none;">Forgot?</a>
        </div>
...
<div style="text-align:center;margin-top:24px;font-size:13px;color:var(--fg-3);">
    ...
    <a href="{{ route('user.register') }}" style="color:#2f54eb;font-weight:500;text-decoration:none;">Create one free</a>
</div>
@endsection
