...
@section('title', 'Pending Works')
@section('page-title', 'Pending Approval')

@section('content')

<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
    <span style="font-size:13px;color:var(--fg-3);">{{ $works->total() }} work{{ $works->total() != 1 ? 's' : '' }} awaiting review</span>
    <a href="{{ route('admin.works.index') }}" class="btn" style="padding:7px 16px;font-size:13px;">
...
<div class="jobstation-card" style="margin-bottom:14px;overflow:hidden;">
    <div style="display:flex;gap:16px;padding:20px;">
        ...
        <div style="flex:1;min-width:0;">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:8px;">
                <h3 style="font-size:16px;font-weight:600;color:var(--fg);line-height:1.3;margin:0;">{{ $work->title }}</h3>
            ...
            <div style="display:flex;flex-wrap:wrap;gap:16px;font-size:12px;color:var(--fg-3);margin-bottom:10px;">
                ...
                @endif
                <span>By <strong style="color:var(--fg-2);">{{ $work->poster_type == 1 ? 'Admin' : ($work->poster?->username ?? '?') }}</strong> · {{ $work->created_at->diffForHumans() }}</span>
            </div>
            ...
            <div style="font-size:13px;color:var(--fg-3);line-height:1.6;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">
                {!! nl2br(e(Str::limit(strip_tags($work->description), 200))) !!}
            </div>
        </div>
    </div>
    ...
    <div style="display:flex;align-items:center;gap:10px;padding:14px 20px;border-top:1px solid var(--border);">
        <form method="POST" action="{{ route('admin.works.approve', $work->id) }}">
            @csrf
        ...
        <a href="{{ route('admin.works.show', $work->id) }}"
...
<div class="jobstation-card" style="padding:60px;text-align:center;">
    ...
    <a href="{{ route('admin.works.index') }}" class="btn" style="margin-top:16px;display:inline-flex;padding:8px 20px;font-size:13px;">Back to Works</a>
</div>
@endforelse
...
<div style="margin-top:16px;display:flex;justify-content:flex-end;gap:6px;">
    ...
    <a href="{{ $works->previousPageUrl() }}" class="btn" style="padding:7px 16px;font-size:12.5px;">Prev</a>
    @endif
    @if($works->hasMorePages())
    <a href="{{ $works->nextPageUrl() }}" class="btn" style="padding:7px 16px;font-size:12.5px;">Next</a>
    @endif
</div>
@endif
