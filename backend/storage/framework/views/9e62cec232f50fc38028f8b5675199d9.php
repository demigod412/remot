...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:18px;">
    ...
    <div class="jobstation-card" style="padding:18px;">
        ...
        <div class="mono" style="font-size:24px;font-weight:600;letter-spacing:-0.6px;"><?php echo e(number_format($totalUsers)); ?></div>
    ...
    <div class="jobstation-card" style="padding:18px;">
        ...
        <div class="mono" style="font-size:24px;font-weight:600;letter-spacing:-0.6px;color:#22C55E;"><?php echo e(number_format($activeUsers)); ?></div>
        <div style="font-size:11.5px;color:var(--fg-3);margin-top:4px;">
            <?php echo e($totalUsers > 0 ? round(($activeUsers / $totalUsers) * 100, 1) : 0); ?>% of total
        </div>
    </div>

    <div class="jobstation-card" style="padding:18px;">
        <div class="label" style="margin-bottom:6px;">Suspended</div>
        <div class="mono" style="font-size:24px;font-weight:600;letter-spacing:-0.6px;color:#EF4444;"><?php echo e(number_format($bannedUsers)); ?></div>
        <div style="font-size:11.5px;color:var(--fg-3);margin-top:4px;">
            <?php echo e($totalUsers > 0 ? round(($bannedUsers / $totalUsers) * 100, 1) : 0); ?>% of total
        </div>
    </div>

    <div class="jobstation-card" style="padding:18px;">
        <div class="label" style="margin-bottom:6px;">KYC pending</div>
        <div class="mono" style="font-size:24px;font-weight:600;letter-spacing:-0.6px;color:#60A5FA;"><?php echo e(number_format($kycPending)); ?></div>
        <div style="font-size:11.5px;margin-top:4px;">
            <a href="<?php echo e(route('admin.users.kyc')); ?>" style="color:var(--accent);text-decoration:none;">Review →</a>
        </div>
    </div>
...
<div class="jobstation-card" style="padding:0;overflow:hidden;">
    ...
    <div style="padding:14px 18px;border-bottom:1px solid var(--border);display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
        <form method="GET" action="<?php echo e(route('admin.users.index')); ?>" style="display:flex;gap:10px;align-items:center;flex:1;flex-wrap:wrap;">
            ...
            <div style="position:relative;flex:1;min-width:200px;max-width:360px;">
                ...
                <input name="search" value="<?php echo e(request('search')); ?>" placeholder="Search by name, email, @handle…"
            ...
            <select name="status" style="width:auto;padding:7px 10px;font-size:12px;">
                ...
                <option value="0" <?php echo e(request('status')=='0' ? 'selected' : ''); ?>>Suspended</option>
            </select>

            <select name="kyc" style="width:auto;padding:7px 10px;font-size:12px;">
                <option value="">All KYC</option>
                <option value="1" <?php echo e(request('kyc')=='1' ? 'selected' : ''); ?>>Verified</option>
                <option value="2" <?php echo e(request('kyc')=='2' ? 'selected' : ''); ?>>Pending</option>
                <option value="0" <?php echo e(request('kyc')=='0' ? 'selected' : ''); ?>>Unverified</option>
            </select>

            <button type="submit" class="btn btn-sm">Filter</button>
            <?php if(request()->hasAny(['search','status','kyc'])): ?>
            <a href="<?php echo e(route('admin.users.index')); ?>" class="btn btn-sm btn-ghost">Clear</a>
            <?php endif; ?>
        </form>

        <a href="<?php echo e(route('admin.users.notify.bulk')); ?>" class="btn btn-sm">
    ...
    <div style="display:grid;grid-template-columns:28px 2fr 90px 100px 110px 110px 100px 80px;gap:14px;padding:12px 18px;align-items:center;border-bottom:1px solid var(--border);font-size:12.5px;">
        ...
        <div style="min-width:0;">
            <div style="font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?php echo e($user->fullname); ?></div>
            <div style="font-size:10.5px;color:var(--fg-3);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?php echo e($user->email); ?></div>
        </div>

        <span class="mono" style="font-size:11px;color:<?php echo e($kycColor); ?>;"><?php echo e($kycLabel); ?></span>

        <span class="mono"><?php echo e(number_format($subCount)); ?></span>

        <span class="mono" style="color:var(--coin);"><?php echo e(coinSymbol()); ?><?php echo e(number_format($user->coin_balance)); ?></span>

        <span style="font-size:11px;color:var(--fg-3);"><?php echo e($user->created_at->format('M j, Y')); ?></span>

        <span class="status-pill <?php echo e($statusClass); ?>" style="width:fit-content;"><?php echo e($statusLabel); ?></span>

        <div style="display:flex;gap:6px;justify-content:flex-end;">
            <a href="<?php echo e(route('admin.users.show', $user->id)); ?>" class="btn btn-sm">View</a>
        </div>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/admin/users/index.blade.php ENDPATH**/ ?>