<?php $__env->startSection('title', gs()->site_name ?? config('app.name')); ?>
...
<script>
window._heroTyper = <?php echo json_encode($heroTyper, 15, 512) ?>;
</script>


<section style="position:relative; overflow:hidden; padding:80px 40px 100px; background:#fff;">
    
    <?php $heroV = @filemtime(public_path('videos/jobstation-hero.mp4')) ?: 1; ?>
    <video class="hero-bg-video" autoplay muted loop playsinline preload="auto"
           poster="<?php echo e(asset('videos/jobstation-hero.jpg')); ?>?v=<?php echo e($heroV); ?>"
           style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover; z-index:0; pointer-events:none;">
        <source src="<?php echo e(asset('videos/jobstation-hero.mp4')); ?>?v=<?php echo e($heroV); ?>" type="video/mp4">
    </video>
    ...
    <div style="position:relative; z-index:2; max-width:1280px; margin:0 auto; display:grid; grid-template-columns:1.1fr 1fr; gap:80px; align-items:center;" class="hero-grid">
        ...
        <div>
            <div class="anim-hero-badge" style="display:inline-flex; align-items:center; gap:8px; padding:5px 12px 5px 6px; border-radius:999px; background:#fff; border:1px solid var(--border); font-size:12.5px; color:var(--muted); margin-bottom:28px; box-shadow:0 1px 3px rgba(10,10,11,0.06);">
                <span style="background:var(--accent-soft); color:var(--accent); padding:2px 8px; border-radius:999px; font-weight:600; font-size:11px;"><?php echo e(__('NEW')); ?></span>
                <?php echo e(__('Contracts are live — long-term private work →')); ?>
            ...
            <p class="anim-hero-p" style="font-size:17px; color:var(--muted); line-height:1.55; max-width:480px; margin:0 0 36px;">
                <?php echo e(__('Job Station is one marketplace for three kinds of work — instant microtasks, hiring jobs, and long-term contracts. Find paid work in seconds or hire the right person in hours.')); ?>

            </p>
            <div class="anim-hero-actions" style="display:flex; gap:12px; margin-bottom:40px; flex-wrap:wrap;">
                <a href="<?php echo e(route('works.index')); ?>" class="btn btn-primary" style="padding:12px 20px; font-size:14px;">
                    <?php echo e(__('Browse jobs')); ?>
                ...
                <a href="<?php echo e(route('user.works.index')); ?>" class="btn btn-secondary" style="padding:12px 20px; font-size:14px;"><?php echo e(__('Post a job')); ?></a>
            </div>
            <div class="anim-hero-stats" style="display:flex; gap:36px; padding-top:28px; border-top:1px solid var(--border); flex-wrap:wrap;">
                <div>
                    <div class="mono counter" data-target="<?php echo e($stats['completed']); ?>" style="font-size:22px; font-weight:600; letter-spacing:-0.5px; color:var(--text);"><?php echo e(number_format($stats['completed'])); ?></div>
                    <div style="font-size:12px; color:var(--muted); margin-top:2px;"><?php echo e(__('tasks completed')); ?></div>
                </div>
                <div>
                    <div class="mono" style="font-size:22px; font-weight:600; letter-spacing:-0.5px; color:#F5D547;"><?php echo e(coinSymbol()); ?> <?php echo e(number_format($stats['completed'] * 8.5)); ?></div>
                    <div style="font-size:12px; color:var(--muted); margin-top:2px;"><?php echo e(__('paid out to workers')); ?></div>
                </div>
                <div>
                    <div class="mono counter" data-target="<?php echo e($stats['workers']); ?>" style="font-size:22px; font-weight:600; letter-spacing:-0.5px; color:var(--text);"><?php echo e(number_format($stats['workers'])); ?></div>
                    <div style="font-size:12px; color:var(--muted); margin-top:2px;"><?php echo e(__('active workers')); ?></div>
                </div>
            </div>
        </div>
        ...
        <div style="position:relative; height:520px;" class="hero-cards">
            ...
            ?>
            <?php if($heroWorks->isNotEmpty()): ?>
                <?php $__currentLoopData = $heroWorks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $work): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="hero-float-card" style="position:absolute; top:<?php echo e($positions[$i][0]); ?>; left:<?php echo e($positions[$i][1]); ?>; width:260px; padding:16px; box-shadow:0 20px 60px -20px rgba(10,10,11,0.15); animation-delay:<?php echo e($floatDelays[$i]); ?>;" class="card">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                        <span class="chip chip-instant" style="font-size:10px; text-transform:uppercase;">⚡ <?php echo e(__('Instant')); ?></span>
                        <span class="mono" style="font-size:11px; color:var(--muted);"><?php echo e($work->time_limit ?? rand(5,45)); ?><?php echo e(__('min')); ?></span>
                    </div>
                    <div style="font-size:14px; font-weight:500; line-height:1.35; margin-bottom:14px; color:var(--text);"><?php echo e(Str::limit($work->title, 52)); ?></div>
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <span class="mono" style="font-size:14px; font-weight:600; color:#F5D547;"><?php echo e(coinSymbol()); ?><?php echo e($work->coins_per_worker); ?></span>
                        <span style="font-size:11px; color:var(--muted);"><?php echo e($work->category?->name); ?></span>
                    </div>
                </div>
            ...
            <?php else: ?>
                ...
                ]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as [$t,$r,$c,$top,$left,$delay]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="hero-float-card card" style="position:absolute; top:<?php echo e($top); ?>; left:<?php echo e($left); ?>; width:260px; padding:16px; box-shadow:0 20px 60px -20px rgba(10,10,11,0.15); animation-delay:<?php echo e($delay); ?>;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                        <span class="chip chip-instant" style="font-size:10px; text-transform:uppercase;">⚡ <?php echo e(__('Instant')); ?></span>
                        <span class="mono" style="font-size:11px; color:var(--muted);">14<?php echo e(__('min')); ?></span>
                    </div>
                    <div style="font-size:14px; font-weight:500; line-height:1.35; margin-bottom:14px; color:var(--text);"><?php echo e($t); ?></div>
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <span class="mono" style="font-size:14px; font-weight:600; color:#F5D547;"><?php echo e(coinSymbol()); ?><?php echo e($r); ?></span>
                        <span style="font-size:11px; color:var(--muted);"><?php echo e($c); ?></span>
                    </div>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php endif; ?>
...
<section id="how-it-works" style="padding:100px 40px; border-top:1px solid var(--border); background:#fff;">
    <div style="max-width:1200px; margin:0 auto;">
        <div data-reveal style="display:flex; align-items:flex-end; justify-content:space-between; margin-bottom:60px; flex-wrap:wrap; gap:20px;">
            <div>
                <div style="font-size:11px; font-weight:600; letter-spacing:0.08em; text-transform:uppercase; color:var(--accent); margin-bottom:12px;"><?php echo e(__('How Job Station works')); ?></div>
                <h2 style="font-size:clamp(28px,3.5vw,48px); font-weight:600; letter-spacing:-1.5px; margin:0; max-width:600px; color:var(--text); line-height:1.1;">
                    <?php echo e(__('Three systems, one wallet, zero friction.')); ?>

                </h2>
            </div>
            <a href="<?php echo e(route('contact')); ?>" class="btn btn-secondary" style="font-size:13px;">
        ...
        <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:24px;" class="three-col">
            ...
            ]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => [$n,$icon,$title,$color,$bullets]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="card reveal-card" data-reveal data-delay="<?php echo e($idx * 120); ?>" style="padding:28px;">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:28px;">
                    <div style="width:44px; height:44px; border-radius:11px; display:flex; align-items:center; justify-content:center; font-size:20px; background:<?php echo e($color); ?>22; color:<?php echo e($color); ?>;"><?php echo e($icon); ?></div>
                    <span class="mono" style="font-size:11px; color:#A1A1AA;"><?php echo e($n); ?></span>
                </div>
                <h3 style="font-size:22px; font-weight:600; margin:0 0 20px; letter-spacing:-0.4px; color:var(--text);"><?php echo e($title); ?></h3>
                <ul style="list-style:none; padding:0; margin:0; display:flex; flex-direction:column; gap:12px;">
                    ...
                    <li style="font-size:13.5px; color:var(--muted); display:flex; gap:10px; align-items:flex-start;">
                        <svg width="16" height="16" viewBox="0 0 18 18" fill="none" stroke="<?php echo e($color); ?>" stroke-width="1.8" stroke-linecap="round" style="flex-shrink:0; margin-top:2px;"><path d="M3 9l4 4 8-8"/></svg>
                        <?php echo e($b); ?>
...
<section style="padding:80px 40px; border-top:1px solid var(--border);">
    <div style="max-width:1200px; margin:0 auto;">
        <div data-reveal style="display:flex; align-items:center; gap:12px; margin-bottom:28px;">
            ...
            <span style="font-size:11px; font-weight:600; letter-spacing:0.08em; text-transform:uppercase; color:var(--urgent);"><?php echo e(__('Live right now')); ?></span>
            <div style="flex:1; height:1px; background:var(--border);"></div>
            <span style="font-size:12.5px; color:var(--muted);"><?php echo e($stats['works']); ?> <?php echo e(__('active tasks')); ?></span>
        </div>
        <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:14px;" class="four-col">
            <?php $__empty_1 = true; $__currentLoopData = $featured->take(4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $work): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <a href="<?php echo e(route('works.show', $work->slug)); ?>" style="text-decoration:none; display:block;" data-reveal data-delay="<?php echo e($idx * 80); ?>">
                <div class="card" style="padding:14px; transition:transform .2s cubic-bezier(.34,1.56,.64,1), box-shadow .2s ease;" onmouseover="this.style.transform='translateY(-4px) scale(1.01)';this.style.boxShadow='0 12px 32px rgba(10,10,11,0.12)'" onmouseout="this.style.transform='';this.style.boxShadow=''">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                        <span class="chip chip-instant" style="font-size:10px; text-transform:uppercase;">⚡ <?php echo e(__('Instant')); ?></span>
                        <span class="mono" style="font-size:11px; color:var(--muted);"><?php echo e($work->category?->name); ?></span>
                    </div>
                    <div style="font-size:13.5px; font-weight:500; line-height:1.35; margin-bottom:12px; min-height:38px; color:var(--text);"><?php echo e(Str::limit($work->title, 55)); ?></div>
                    <div style="display:flex; justify-content:space-between; align-items:center; padding-top:10px; border-top:1px solid var(--border);">
                        <span class="mono" style="font-size:14px; font-weight:600; color:#F5D547;"><?php echo e(coinSymbol()); ?><?php echo e($work->coins_per_worker); ?></span>
                        <span style="font-size:11px; font-weight:500; color:var(--accent);"><?php echo e(__('Start →')); ?></span>
                    </div>
                </div>
            </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            <?php $__currentLoopData = [[__('Find business address on website'),3,__('Research')],[__('Proofread product page copy'),8,__('Writing')],[__('Test checkout flow on iOS'),15,__('QA')],[__('Categorize 50 support tickets'),22,__('Data')]]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => [$t,$r,$c]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="card" style="padding:14px;" data-reveal data-delay="<?php echo e($idx * 80); ?>">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    <span class="chip chip-instant" style="font-size:10px; text-transform:uppercase;">⚡ <?php echo e(__('Instant')); ?></span>
                    <span style="font-size:11px; color:var(--muted);"><?php echo e($c); ?></span>
                </div>
                <div style="font-size:13.5px; font-weight:500; line-height:1.35; margin-bottom:12px; min-height:38px; color:var(--text);"><?php echo e($t); ?></div>
                <div style="display:flex; justify-content:space-between; align-items:center; padding-top:10px; border-top:1px solid var(--border);">
                    <span class="mono" style="font-size:14px; font-weight:600; color:#F5D547;"><?php echo e(coinSymbol()); ?><?php echo e($r); ?></span>
                    <a href="<?php echo e(route('works.index')); ?>" style="font-size:11px; font-weight:500; color:var(--accent);"><?php echo e(__('Start →')); ?></a>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php endif; ?>
...
<section style="padding:60px 40px; border-top:1px solid var(--border); background:#fff;">
    <div style="max-width:1200px; margin:0 auto;">
        <div data-reveal style="text-align:center; font-size:12px; color:var(--muted); text-transform:uppercase; letter-spacing:0.1em; margin-bottom:36px;">
            <?php echo e(__('Trusted by teams at 8,000+ companies')); ?>
        ...
        <div style="display:grid; grid-template-columns:repeat(6,1fr); border-top:1px solid var(--border); border-left:1px solid var(--border);" class="logo-grid">
            <?php $__currentLoopData = ['Parcel.io','Halo Studio','Delta','Framework','Pillar','Matter','Groundtruth','Loop Media','Astra','Tessera','Orbit','Kindred']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $logo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="logo-cell" data-reveal data-delay="<?php echo e(($idx % 6) * 60); ?>" style="padding:28px 20px; border-right:1px solid var(--border); border-bottom:1px solid var(--border); display:flex; align-items:center; justify-content:center; color:#A1A1AA; font-size:15px; font-weight:500; letter-spacing:-0.3px; transition:color .2s, background .2s;" onmouseover="this.style.color='var(--text)';this.style.background='rgba(47,84,235,0.04)'" onmouseout="this.style.color='#A1A1AA';this.style.background=''">
                <?php echo e($logo); ?>
...
<section style="padding:100px 40px; border-top:1px solid var(--border);">
    <div style="max-width:1200px; margin:0 auto;">
        <div data-reveal style="display:flex; align-items:flex-end; justify-content:space-between; margin-bottom:48px; flex-wrap:wrap; gap:20px;">
            <div>
                <div style="font-size:11px; font-weight:600; letter-spacing:0.08em; text-transform:uppercase; color:var(--accent); margin-bottom:12px;"><?php echo e(__('Platform health')); ?></div>
                <h2 style="font-size:clamp(28px,3.5vw,48px); font-weight:600; letter-spacing:-1.5px; margin:0; max-width:620px; color:var(--text); line-height:1.1;">
                    <?php echo e(__('The numbers behind the marketplace.')); ?>

                </h2>
            </div>
            <span style="font-size:12.5px; color:var(--muted);"><?php echo e(__('Updated live · all time unless noted')); ?></span>
        </div>
        <div style="display:grid; grid-template-columns:repeat(4,1fr); border-top:1px solid var(--border); border-left:1px solid var(--border);" class="four-col stats-grid">
            ...
            ]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => [$v,$l,$s,$target]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div data-reveal data-delay="<?php echo e(($idx % 4) * 80); ?>" style="padding:32px 28px; border-right:1px solid var(--border); border-bottom:1px solid var(--border);">
                <div class="mono<?php echo e($target ? ' counter' : ''); ?>"<?php echo e($target ? ' data-target="'.$target.'"' : ''); ?> style="font-size:clamp(22px,2.5vw,34px); font-weight:600; letter-spacing:-1.2px; line-height:1; margin-bottom:12px; color:var(--text);"><?php echo e($v); ?></div>
                <div style="font-size:13px; color:var(--text); font-weight:500; margin-bottom:4px;"><?php echo e($l); ?></div>
                <div style="font-size:11.5px; color:var(--muted);"><?php echo e($s); ?></div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    </div>
...
<section style="padding:100px 40px; border-top:1px solid var(--border); background:#fff;">
    <div style="max-width:1200px; margin:0 auto;">
        <div data-reveal style="display:flex; align-items:flex-end; justify-content:space-between; margin-bottom:48px; flex-wrap:wrap; gap:20px;">
            <div>
                <div style="font-size:11px; font-weight:600; letter-spacing:0.08em; text-transform:uppercase; color:var(--accent); margin-bottom:12px;"><?php echo e(__('Loved by workers and clients')); ?></div>
                <h2 style="font-size:clamp(28px,3.5vw,48px); font-weight:600; letter-spacing:-1.5px; margin:0; max-width:620px; color:var(--text); line-height:1.05;">
                    <?php echo e(__('The marketplace people actually come back to.')); ?>
            ...
            <div style="display:flex; align-items:center; gap:14px;">
                ...
                <div>
                    ...
                    <div style="font-size:11px; color:var(--muted);"><?php echo e(__('Avg across 12,400 reviews')); ?></div>
                </div>
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1.4fr 1fr 1fr; gap:18px;" class="testi-grid">
            <div class="card" data-reveal data-delay="0" style="padding:36px; display:flex; flex-direction:column; gap:20px;">
                ...
                <p style="font-size:20px; line-height:1.45; font-weight:500; letter-spacing:-0.3px; margin:0; color:var(--text);">
                    <?php echo e(__('I earned my first JC500 in a weekend doing QA tasks during my commute. Three months later I landed a full hiring gig through the same platform — same profile, same wallet.')); ?>
                ...
                <div style="display:flex; align-items:center; gap:12px; padding-top:20px; border-top:1px solid var(--border); margin-top:auto;">
                    ...
                    <div style="flex:1;">
                        ...
                        <div style="font-size:11.5px; color:var(--muted);"><?php echo e(__('QA Engineer · L2 verified')); ?></div>
                    </div>
                    <div style="text-align:right;">
                        <div class="mono" style="font-size:15px; font-weight:600; color:#F5D547;"><?php echo e(coinSymbol()); ?> 28,463</div>
                        <div style="font-size:10.5px; color:var(--muted);"><?php echo e(__('Lifetime earnings')); ?></div>
                    </div>
                </div>
            </div>
            <?php $__currentLoopData = [
            ...
            ]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => [$init,$name,$role,$quote,$meta,$color]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="card" data-reveal data-delay="<?php echo e(($idx + 1) * 120); ?>" style="padding:24px; display:flex; flex-direction:column; gap:16px;">
                <div style="font-size:13px; color:#F5D547;">★★★★★</div>
                <p style="font-size:14px; line-height:1.55; color:var(--muted); margin:0; flex:1;">"<?php echo e($quote); ?>"</p>
                <div style="display:flex; align-items:center; gap:10px; padding-top:16px; border-top:1px solid var(--border);">
                    <div style="width:32px; height:32px; border-radius:50%; background:linear-gradient(135deg,<?php echo e($color); ?>,<?php echo e($color); ?>99); display:flex; align-items:center; justify-content:center; color:white; font-weight:600; font-size:12px; flex-shrink:0;"><?php echo e($init); ?></div>
                    <div style="flex:1; min-width:0;">
                        <div style="font-size:12.5px; font-weight:500; color:var(--text);"><?php echo e($name); ?></div>
                        <div style="font-size:10.5px; color:var(--muted);"><?php echo e($role); ?></div>
                    </div>
                    <span style="font-size:10px; color:var(--accent); padding:3px 7px; border-radius:4px; background:var(--accent-soft); white-space:nowrap; flex-shrink:0;"><?php echo e($meta); ?></span>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        ...
        <div style="margin-top:18px; display:grid; grid-template-columns:repeat(3,1fr); gap:18px;" class="three-col">
            ...
            ]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => [$init,$name,$role,$color,$quote]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="card" data-reveal data-delay="<?php echo e($idx * 100); ?>" style="padding:20px; display:flex; gap:12px; align-items:flex-start;">
                <div style="width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg,<?php echo e($color); ?>,<?php echo e($color); ?>99); display:flex; align-items:center; justify-content:center; color:white; font-weight:600; font-size:13px; flex-shrink:0;"><?php echo e($init); ?></div>
                <div>
                    <p style="font-size:13px; line-height:1.45; margin:0 0 8px; color:var(--text);">"<?php echo e($quote); ?>"</p>
                    <div style="font-size:11px; color:var(--muted);"><?php echo e($name); ?> · <?php echo e($role); ?></div>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
...
<section style="padding:100px 40px; border-top:1px solid var(--border);">
    <div style="max-width:1100px; margin:0 auto; display:grid; grid-template-columns:380px 1fr; gap:80px;" class="faq-grid">
        <div data-reveal>
            <div style="font-size:11px; font-weight:600; letter-spacing:0.08em; text-transform:uppercase; color:var(--accent); margin-bottom:12px;"><?php echo e(__('FAQ')); ?></div>
            <h2 style="font-size:clamp(28px,3vw,44px); font-weight:600; letter-spacing:-1.5px; margin:0 0 20px; line-height:1.05; color:var(--text);"><?php echo e(__('Questions, answered.')); ?></h2>
            <p style="font-size:14.5px; color:var(--muted); line-height:1.55; margin:0 0 24px;">
                <?php echo e(__("Can't find what you're looking for? Our help center has 200+ articles and our support team replies in under 4 hours.")); ?>

            </p>
            <a href="<?php echo e(route('contact')); ?>" class="btn btn-secondary" style="font-size:13px;">
                <?php echo e(__('Visit help center')); ?>
        ...
        <div x-data="{ open: 0 }" data-reveal data-delay="100">
            ...
            ]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => [$q,$a]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div style="border-bottom:1px solid var(--border); <?php echo e($idx===0 ? 'border-top:1px solid var(--border);' : ''); ?>">
                <button @click="open === <?php echo e($idx); ?> ? open = -1 : open = <?php echo e($idx); ?>"
                    ...
                    <span style="font-size:16px; font-weight:500; letter-spacing:-0.2px; color:var(--text);"><?php echo e($q); ?></span>
                    <span style="color:var(--muted); flex-shrink:0; margin-top:2px; transition:transform .18s;" :style="open === <?php echo e($idx); ?> ? 'transform:rotate(45deg)' : ''">
                ...
                <div x-show="open === <?php echo e($idx); ?>" x-transition style="padding-bottom:20px;">
                    <p style="font-size:14px; color:var(--muted); line-height:1.6; margin:0;"><?php echo e($a); ?></p>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
...
<section style="padding:100px 40px; border-top:1px solid var(--border);">
    <div data-reveal style="max-width:1000px; margin:0 auto; text-align:center; padding:80px 60px; background:linear-gradient(135deg,rgba(47,84,235,0.08),#fff); border:1px solid var(--border); border-radius:24px; box-shadow:0 1px 3px rgba(10,10,11,0.06);">
        <h2 style="font-size:clamp(32px,4vw,56px); font-weight:600; letter-spacing:-2px; margin:0 0 20px; color:var(--text);">
            <?php echo e(__('Your next JC is two clicks away.')); ?>

        </h2>
        <p style="font-size:17px; color:var(--muted); max-width:560px; margin:0 auto 32px; line-height:1.55;">
            <?php echo e(__('Sign up free. No listing fees. Get verified in 6 minutes and take your first task.')); ?>

        </p>
        <div style="display:inline-flex; gap:12px; flex-wrap:wrap; justify-content:center;">
            <a href="<?php echo e(route('user.register')); ?>" class="btn btn-primary" style="padding:12px 22px; font-size:14.5px;"><?php echo e(__('Get started free')); ?></a>
            <a href="<?php echo e(route('works.index')); ?>" class="btn btn-secondary" style="padding:12px 22px; font-size:14.5px;"><?php echo e(__('See open jobs')); ?></a>
        </div>
    </div>
</section>
...
<?php echo $__env->make('web.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/web/home.blade.php ENDPATH**/ ?>